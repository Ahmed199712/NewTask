...
class AdminProfileController extends Controller
{
    ...
    {   
        ...
        if( $request->avatar ){
            if( $admin->avatar != 'uploads/avatars/admins/default.png' ){
                unlink($admin->avatar);
            }
            Image::make($request->avatar)->resize(null, 400, function ($constraint) {
                $constraint->aspectRatio();
            })->save('uploads/avatars/admins/' . $request->avatar->hashName());
            $admin->avatar = 'uploads/avatars/admins/' . $request->avatar->hashName();
        }
